...
* Creation
  ...
  (defn sphere
    ...
    ([transform]
     (sphere transform (mr/material)))
    ([]
     (sphere (m/id 4) (mr/material))))
  #+END_SRC
...
* Normal
  ...
  (testing "Computing the normal on a translated sphere"
    (is (t/eq? (t/vector 0. 0.70711 -0.70711)
               (normal-at (sphere (tr/translation 0. 1. 0.))
                          (t/point 0, 1.70711, -0.70711)))))

  (testing "Computing the normal on a scaled sphere"
    (is (t/eq? (t/vector 0. 0.97014 -0.24254)
               (normal-at (sphere (tr/scaling 1. 0.5 1.))
                          (t/point 0, 0.70711, -0.70711)))))
  #+END_SRC

  To calculate the world-normal, we must :
  - first transform the intersection point into object-world.
  - then calculate the normal in object-world easily.
  - we must then transform this normal back, using the transpose inverse of the transformation matrix of the sphere.
  ...
  #+NAME: spheres_norm
  #+BEGIN_SRC clojure
  (defn normal-at [{:keys [inverse-t trans-inverse-t]} world-p]
    (let [object-p (m/mul inverse-t world-p)
          object-normal (t/sub object-p (t/point 0. 0. 0.))
...
* Example
  ...
  #+BEGIN_SRC clojure :exports code :tangle ../dev/rt_clj/sphere_ray_example.clj

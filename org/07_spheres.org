...
* Normal
  ...
  #+NAME: spheres_norm
  #+BEGIN_SRC clojure
  (defn normal [{:keys [inverse-t trans-inverse-t]} world-p]
    (let [object-p (m/mul inverse-t world-p)
          object-normal (t/sub object-p t/origin)
